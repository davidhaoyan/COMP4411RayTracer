...
static void processObject( Obj *obj, Scene *scene, mmap& materials )
{
	...
	if( name == "directional_light" ) {
		...
		if (hasField(child, "constant_attenuation_coeff")) {
			pointLight->a = getField(child, "constant_attenuation_coeff")->getScalar();
		}
		if (hasField(child, "linear_attenuation_coeff")) {
			pointLight->b = getField(child, "linear_attenuation_coeff")->getScalar();
		}
		if (hasField(child, "quadratic-atttenuation-coeff")) {
			pointLight->c = getField(child, "quadratic_attenuation_coeff")->getScalar();
		}
		...
		scene->ambientColor = tupleToVec(getColorField(child));
	} else if (name == "sphere" ||
				name == "box" ||
				name == "cylinder" ||
				name == "cone" ||
